...
const MusicShaderCanvas = () => {
  ...
  useEffect(() => {
    ...
    const checkAudioFile = async () => {
      const audioPath = `${window.location.origin}/sounds/beat.mp3`;
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      const float BAND_WIDTH = 0.15;            // Ancho de cada banda de frecuencia
  ...
  const handleClick = async () => {
    try {
      ...
      if (!audioSrc || audioSrc === window.location.origin + '/' || !audioSrc.includes('beat.mp3')) {
        const newSrc = `${window.location.origin}/sounds/beat.mp3`;
  ...
  return (
    <div className="relative w-full h-full overflow-visible">
      ...
      <audio 
        ...
        className="hidden"
        src={`${window.location.origin}/sounds/beat.mp3`}
        loop
        preload="auto"
